...
if __name__ == "__main__":
    with open(sys.argv[1]) as f:
        ...
        for i in range(number_of_tasks):
            processing_times[i], release_times[i], deadlines[i] = [int(x) for x in f.readline().split()]
    scheduled_tasks = []
    ...
    branch_and_bound(processing_times, release_times, deadlines, scheduled_tasks, tasks_to_be_scheduled, best_solution, upper_bound)
